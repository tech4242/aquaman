The Aquaman plugin isolates OpenClaw API keys and channel tokens in a separate proxy process — the agent only ever sees a sentinel base URL and a placeholder key, never a real credential. The OpenClaw plugin scope is stable release-over-release: same proxy, same vault backends, same audit and policy engine. Each release is verified against the OpenClaw Gateway version recorded in `package.json` (`openclaw.build` / `openclaw.compat`). The repo ships a runnable conformance suite under `test/compliance/` mapped to MITRE ATLAS v5.4.0 + NIST SP 800-53 Rev 5 controls (run via `npm test` or `vitest run test/compliance/`).
