v0.12.0 adds an aquaman-coder sibling package for AI coding agents (Claude Code shipping; Codex / OpenCode / Cursor planned). The OpenClaw plugin scope is unchanged from v0.11.5 — same proxy, same vault backends, same audit and policy engine. The repo now also ships a runnable conformance suite under `test/compliance/` mapped to MITRE ATLAS v5.4.0 + NIST SP 800-53 Rev 5 controls (run via `npm test` or `vitest run test/compliance/`).
