...
// Compliance commands — runs the conformance suite mapped to ATLAS / NIST controls
const compliance = program.command('compliance').description('Compliance conformance tests (MITRE ATLAS, NIST 800-53)');
...
compliance
  ...
  .action(async (opts: { json?: boolean; framework: string }) => {
    ...
    // Resolve repo root: CLI binary lives at packages/proxy/dist/cli/index.js
    // or packages/proxy/src/cli/index.ts. Two parents up from dist/cli/.
    const repoCandidates = [
      path.resolve(__cliDirname, '../../..'),  // dist/cli -> proxy -> packages -> repo
      path.resolve(__cliDirname, '../../../..'), // dist/cli -> src/cli -> proxy -> ...
      process.cwd(),
    ];
    ...
    let json: any = null;
    try {
      // vitest may emit log lines before the JSON object — find first '{'
      const firstBrace = result.stdout.indexOf('{');
      if (firstBrace >= 0) json = JSON.parse(result.stdout.slice(firstBrace));
    } catch {
      // fall through; we'll report failure
    }

    // Map test files → framework control IDs
    const fileToControls: Record<string, { framework: 'atlas' | 'nist'; ids: string[] }> = {
      't0055-unsecured-credentials.test.ts': { framework: 'atlas', ids: ['AML.T0055'] },
      't0012-valid-accounts.test.ts': { framework: 'atlas', ids: ['AML.T0012'] },
      't0062-exfiltration-via-tool.test.ts': { framework: 'atlas', ids: ['AML.T0062'] },
      't0090-os-credential-dumping.test.ts': { framework: 'atlas', ids: ['AML.T0090'] },
      'ia-5-authenticator-mgmt.test.ts': { framework: 'nist', ids: ['IA-5'] },
      'ac-3-access-enforcement.test.ts': { framework: 'nist', ids: ['AC-3', 'AC-6'] },
      'au-10-tamper-evident.test.ts': { framework: 'nist', ids: ['AU-2', 'AU-9', 'AU-10'] },
      'sc-28-protection-at-rest.test.ts': { framework: 'nist', ids: ['SC-12', 'SC-28'] },
      'si-10-input-validation.test.ts': { framework: 'nist', ids: ['SI-10'] },
    };

    interface ControlEvidence {
      status: 'pass' | 'fail' | 'unknown';
      framework: 'atlas' | 'nist';
      file: string;
      passed: number;
      failed: number;
      tests: string[];
    ...
    const evidence: Record<string, ControlEvidence> = {};

    if (json?.testResults) {
      for (const tr of json.testResults) {
        const base = path.basename(tr.name || '');
        const mapping = fileToControls[base];
        if (!mapping) continue;

        const passed = (tr.assertionResults || []).filter((a: any) => a.status === 'passed').length;
        const failed = (tr.assertionResults || []).filter((a: any) => a.status === 'failed').length;
        const titles = (tr.assertionResults || []).map((a: any) => a.fullName || a.title);
    ...
    console.log(aqua('Aquaman compliance check'));
    console.log(`Framework filter: ${framework}`);
    console.log(`Repo: ${repoRoot}`);
    console.log('');

    const ids = Object.keys(evidence).sort();
    if (ids.length === 0) {
      console.log('  No compliance test results parsed.');
    ...
    for (const id of ids) {
      ...
      console.log(`  ${mark} ${id.padEnd(12)} ${e.passed}/${e.passed + e.failed} tests passed  (${e.file})`);
    }
    console.log('');
    console.log(`Overall: ${result.status === 0 ? 'PASS' : 'FAIL'}`);
    process.exit(result.status === 0 ? 0 : 1);
  });
