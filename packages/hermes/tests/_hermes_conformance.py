...
#
# Upstream: NousResearch/hermes-agent  tests/secret_sources/conformance.py
# Retrieved: 2026-08-03 from refs/heads/main
# Why vendored: the kit is repo-only — the hermes-agent PyPI wheel ships
#   agent/secret_sources/ but not tests/, so `pip install hermes-agent` cannot
#   provide it. The checks below run against the REAL ABC + orchestrator from
#   the installed wheel; only the kit source is copied.
# Refresh: re-download from upstream main and re-run this suite.
# ---------------------------------------------------------------------------
