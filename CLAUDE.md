...
**⛔ OPEN BLOCKER (found 2026-09-18, unfixed in v0.15.0): OpenClaw provider traffic bypasses the proxy.** On real gateways (2026.7.33 and 2026.9.1), agent model calls go through OpenClaw's `provider-transport-fetch` with its own undici dispatcher (`dispatcher=new` in the gateway log). It neither honors the sentinel `ANTHROPIC_BASE_URL` env nor passes through our `globalThis.fetch` interceptor. The SecretRef placeholder goes straight to `https://api.anthropic.com` and fails with `invalid x-api-key`, and nothing lands in the aquaman audit log. Setting `models.providers.anthropic.baseUrl = http://aquaman.local/anthropic` routes to the sentinel, but the transport does its own DNS (`getaddrinfo ENOTFOUND aquaman.local`). No real key leaks (the gateway only ever holds the placeholder), but the LLM-provider half of the OpenClaw integration doesn't function on current gateways. The CI e2e never sends real provider traffic, so it can't catch this. Likely fix direction: point `models.providers.<svc>.baseUrl` at a real loopback listener (the Hermes transport: `http://127.0.0.1:<port>/<svc>`) and have the SecretRef resolver return the loopback token as the "api key". Or use OpenClaw's provider-transport proxy settings (`proxy=none policy=custom` in the log hints at a configurable proxy). Needs a design decision. Channel traffic (Telegram/Slack/…) through the interceptor was **not** re-verified against a live 7.33/9.1 gateway.
